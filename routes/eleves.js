  ...
  try {
    ...
    let sql = `
      SELECT e.id, e.nom, e.prenom, e.massar, e.date_naissance,
             e.numero_classe, e.points, e.photo, e.suspect_triche, c.nom AS classe_nom, c.id AS classe_id
      FROM eleves e
...
// PATCH /api/eleves/:id/suspect
// Marque/démarque un élève comme suspect de triche.
// Lu par GestiTest pour appliquer une correction IA plus stricte sur ses copies.
router.patch('/:id/suspect', requireTeacher, async (req, res) => {
  try {
    const { suspect_triche } = req.body;
    if (suspect_triche === undefined) return res.status(400).json({ error: 'suspect_triche requis.' });

    const [[eleve]] = await db.query('SELECT id FROM eleves WHERE id = ?', [req.params.id]);
    if (!eleve) return res.status(404).json({ error: 'Élève introuvable.' });

    const value = suspect_triche ? 1 : 0;
    await db.query('UPDATE eleves SET suspect_triche = ? WHERE id = ?', [value, eleve.id]);

    res.json({ success: true, suspect_triche: value });
  } catch (err) { console.error(err); res.status(500).json({ error: 'Erreur serveur.' }); }
});
